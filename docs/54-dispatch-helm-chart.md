...
| UPGRADE_TYPE          | string | Which SemVer part to increment: `auto` (default), `major`, `minor`, `patch` or `prerelease` - forwarded verbatim to the chart repository, where [`update-helm-chart.yml`](./53-update-helm-chart.md#auto-mode) derives the level from the appVersion delta (falling back to `patch` with a warning when there is no delta to read) | No       | auto                   |
| PRERELEASE_IDENTIFIER | string | Identifier used when the bump enters the prerelease flow - `UPGRADE_TYPE: prerelease`, or `auto` with a prerelease `APP_VERSION` (e.g. `rc`)                                                                              | No       | rc                     |
| AUTOMERGE_PRERELEASE  | bool   | Ask the chart repository to merge its update PR when the bump is a prerelease                                                     | No       | false                  |
| AUTOMERGE_RELEASE     | bool   | Ask the chart repository to merge its update PR when the bump is not a prerelease                                                 | No       | false                  |
