...
- **This assumes the promotion preserves commits** (merge or rebase-merge). A **squash** merge of `PRERELEASE_BRANCH` → `RELEASE_BRANCH` breaks that property: the original commits are no longer ancestors, the rebase replays all of them, and conflicts become the norm.
