...
- **Detected package manager is allowlisted.** The `packageManager` field is read out of the repository's `package.json`, so on a pull request it is contributor-controlled. The detection step rejects anything that is not `npm`, `pnpm`, `yarn` or `bun` before publishing the value, and the steps that consume it read it through `env:` rather than interpolating it into the shell. An unrecognised value fails the job with a message naming the field.
- **LINT_PATHS** accepts comma- or space-separated paths and is split into argv on those separators only — the shell does not expand globs in it, so `src/**/*.ts` reaches ESLint verbatim and is matched by ESLint's own glob handling. Paths containing spaces cannot be expressed; lint their parent directory instead.
