...
| HELM_DOCS_VERSION     | string | The helm-docs release that regenerates the chart README, as its tag. Built from source at exactly that version and verified against the Go checksum database; `latest` is refused | No       | v1.14.2                |
